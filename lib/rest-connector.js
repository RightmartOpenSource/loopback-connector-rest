...
function RestConnector(baseURL, settings) {
  ...
  var _request = request.defaults(this._settings);

  const NodeCache = require('node-cache');
  const myCache = new NodeCache({stdTTL: 1, checkperiod: 0.9});

  this._request = function(options, callback) {
    if (options.method === 'GET') {
      console.log(options);
      var md5sum = crypto.createHash('md5').update(JSON.stringify(options)).digest('hex');
      console.log(options, md5sum);
      var cached = myCache.get(md5sum);
      if (cached) {
        console.log('Cached response ', cached, md5sum, options);
        callback(null, {statusCode: 200}, cached);
      }
      debug(md5sum);
    }
    const callbackCustom = function(err, req, res) {
      debug('response', res);
      if (!err && md5sum) {
        myCache.set(md5sum.toString(), res);
      }
      callback.apply(this, arguments);
    };
    return _request.call(this, options, callbackCustom);
  };
}
